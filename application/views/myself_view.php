            ...
            <div class="order-list">
                ...
                <div class="nodo">
							<span class="order-1">
								...
								<p class="address">收货地址：<?php if ($order['deliveryMode'] == 'self'){ echo $order['shop'].'(自取)';} else{ echo $order['area'].$order['buildingNum'].'栋'.$order['roomNum'];}?></p>
							...
							<?php }?>
                    <p class="status">总价：<?php echo $order['total']?>&nbsp;&nbsp;&nbsp;&nbsp;收货方式：<?php if ($order['deliveryMode'] == 'self'){ echo '自取';}else{ echo '送货上门';}?>&nbsp;&nbsp;&nbsp;&nbsp;订单状态：
                        <?php
                        if ($order['state'] == 'UNPAID'){ echo '未支付';}
                        ?></p>
                    <hr />
                </div>
                <?php } ?>
